...
The bot implements a daily archival strategy to ensure comprehensive weekly coverage:

1.  **Daily Backup:** Every daily run (e.g., `general_daily`, `tech_daily`) saves its fetched articles to `data/archive/YYYY-MM-DD_{type}.json`.
2.  **Weekly Retrieval:** Weekly runs (e.g., `general_weekly`) load these archived articles (up to 7 days back) to fill gaps in RSS history.
3.  **Isolation:** Archives are tagged by content type (e.g., `tech` runs only load `_tech` archives), ensuring topic purity.
4.  **Cleanup:** Old archive files are automatically deleted after 7 days (configurable via `retention_days`).
